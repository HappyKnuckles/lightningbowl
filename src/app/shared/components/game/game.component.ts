...
export class GameComponent implements OnInit {
  ...
  displayItems = computed((): DisplayItem[] => {
    ...
    const seriesGroupMap = new Map<string, SeriesGroup>();
    ...
    for (const game of sorted) {
      ...
      let group = seriesGroupMap.get(game.seriesId);
      if (!group) {
        group = {
        ...
        seriesGroupMap.set(game.seriesId, group);
    ...
    for (const group of seriesGroupMap.values()) {
      group.games.sort((a, b) => a.date - b.date);
      group.totalScore = group.games.reduce((sum, g) => sum + g.totalScore, 0);
      group.averageScore = group.totalScore / group.games.length;
    }

    return items;
  });

  showingDisplayItems = computed(() => this.displayItems().slice(0, this.loadedCount()));
  ...
  monthHeaders = computed(() => {
    ...
    const formatName = (timestamp: number) => new Date(timestamp).toLocaleDateString(undefined, { month: 'long', year: 'numeric' });
    ...
    headers.set(0, {
      name: formatName(getDate(items[0])),
      count: counts.get(getMonthKey(getDate(items[0]))) || 0,
    });

    for (let i = 1; i < items.length; i++) {
      const currentKey = getMonthKey(getDate(items[i]));
      if (currentKey !== getMonthKey(getDate(items[i - 1]))) {
        headers.set(i, { name: formatName(getDate(items[i])), count: counts.get(currentKey) || 0 });
      }
    }
